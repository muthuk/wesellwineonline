		...
		$text_after_signup = (isset($opts['form']['wpautop']) && $opts['form']['wpautop'] == 1) ? wpautop(wptexturize($text_after_signup)) : $text_after_signup;
		 ...
		 if(!isset($_POST['nsu_submit']) || count($errors) > 0) { //form has not been submitted yet 
			...
			if(isset($opts['mailinglist']['subscribe_with_name']) && $opts['mailinglist']['subscribe_with_name'] == 1) {	
				$output .= "<p><label for=\"nsu-name-$formno\">$name_label</label><input class=\"nsu-field\" id=\"nsu-name-$formno\" type=\"text\" name=\"$name_id\" value=\"$name_value\" onblur=\"if(this.value == '') this.value='$name_value'\"  onfocus=\"if(this.value == '$name_value') this.value=''\"  />";
                                if(isset($errors['name-field'])) $output .= '<span class="nsu-error error notice">'.$errors['name-field'].'</span>';
                                $output .= "</p>";		
			} 
							
			$output .= "<p><label for=\"nsu-email-$formno\">$email_label</label><input class=\"nsu-field\" id=\"nsu-email-$formno\" type=\"text\" name=\"$email_id\" value=\"$email_value\"  onblur=\"if(this.value == '') this.value='$email_value'\" onfocus=\"if(this.value == '$email_value') this.value=''\" />";
